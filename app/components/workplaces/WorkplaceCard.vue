  ...
  <NuxtLink :to="`/workplaces/${workplace._id}`">
    <BaseCard noPadding class="overflow-hidden hover:border-indigo-500/40 transition-all duration-200 cursor-pointer group">
      ...
      <div class="relative h-36 overflow-hidden">
        <!-- Cover image (if set) or gradient fallback -->
        <div
        ...
        <!-- Workplace avatar - bottom left -->
        <div
          :class="`absolute bottom-3 left-3 w-9 h-9 rounded-xl bg-gradient-to-br ${getWorkplaceGradient(workplace.name)} flex items-center justify-center text-xs font-bold text-white shadow-lg ring-2 ring-black/20`"
        >
          {{ initials }}
        </div>

        <!-- Plan + Status badges - bottom right -->
      ...
      <div class="px-4 pt-3 pb-3">
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            ...
            <span class="text-[11px] font-mono text-gray-500 tracking-wide truncate">taskflow.io/{{ workplace.slug }}</span>
          </div>
        </div>

        <!-- Stats -->
        <div class="mt-3 grid grid-cols-2 gap-2">
          <div class="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800/30 border border-slate-700/30">
        ...
        <div class="flex items-center justify-between mt-3 pt-3 border-t border-slate-700/30">
          ...
          <span class="text-[11px] text-gray-600 font-mono">{{ workplace.slug }}</span>
        </div>
      </div>
    </BaseCard>
  </NuxtLink>
