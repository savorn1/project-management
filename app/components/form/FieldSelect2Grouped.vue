    ...
    <div ref="containerRef" class="relative">
      ...
      >
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
          ...
          <svg class="w-4 h-4 text-gray-400 transition-transform" :class="{ 'rotate-180': isOpen }"
        ...
        >
          ...
          <ul class="overflow-y-auto max-h-60 py-1">
            <template v-for="group in filteredGroups" :key="group.label">
              <li class="px-3 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-slate-800/50 sticky top-0">
                {{ group.label }}
              </li>
              <li
                ...
                :class="modelValue === opt.value ? 'bg-indigo-600 text-white' : 'text-gray-200 hover:bg-slate-700'"
                @click="select(opt.value)"
              >
                <svg v-if="modelValue === opt.value" class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li v-if="filteredGroups.every(g => g.options.length === 0)" class="px-3 py-2 text-sm text-gray-500 text-center">
              No options found
            </li>
          </ul>
        </div>
