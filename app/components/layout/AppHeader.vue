<template>
  <header class="h-16 shrink-0 bg-slate-900/80 backdrop-blur-xl border-b border-slate-800/50 flex items-center justify-between px-6 z-20">
    ...
    <div class="flex items-center gap-3">
      ...
      <!-- Settings -->
      <button @click="router.push('/settings')" class="w-10 h-10 bg-slate-800/50 border border-slate-700/50 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-slate-700/50 hover:border-slate-600/50 transition-all duration-200 group">
      ...
      <div class="relative" ref="userMenuRef">
        ...
        >
          ...
          <div class="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0">
            ...
            <div v-else class="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-sm font-semibold">
              {{ userInitials || 'U' }}
            </div>
          </div>
          <!-- User Info -->
